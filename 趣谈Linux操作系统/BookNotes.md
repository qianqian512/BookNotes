...
> 8. 实模式和保护模式，在寻址时的方式都是差不多相同的，都是基于段基址+偏移地址来确定物理地址的；不同的是在定位段基址时方式不同。实模式下直接将段基址保存在寄存器上，而保护模式是存在一张表中，GDT（Global Discriptor Table）。
...
> 13. linux分段机制和分页机制？在内存管理时为什么不用分段，而用分页?<font color="blue">待补充</font>
