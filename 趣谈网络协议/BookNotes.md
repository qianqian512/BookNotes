...
## 第15章：HTTPS协议
#### 1.说明
>（1）HTTPS是基于会话层的SSL协议加密传输的一种网络协议。
>（2）
#### 2.通信过程
>（1）因为SSL是建立在会话层的网络协议，所以首先还要先通过TCP的三次握手建立连接。
>（2）客户端将版本号，自己支持的加密算法和一个随机数RandomC以明文方式发给服务端。
>（3）服务端根据客户端传入的算法中，挑选出一个，然后生成一个RandomS，并连同将自己的身份信息以CA方式返回给客户端（其中CA中包含了网站地址、公钥和签名结构几个重要信息）。
>（4）客户端验证CA的合法性，以及CA包含的网址和正在访问的目标网址是否一致。关于验证CA合法性：1.首先我们的OS会自带一些权威CA机构的公钥；2.CA也会经过层层授信，直到确认到一个可用的RootCA机构。
>（5）CA在获得客户端的信任后，客户端会生成一个随机数Pre-Master（后续对称加密算法的秘钥），并用公钥加密，发给服务端。
>（6）服务端收到客户端的消息，用自己的私钥解出Pre-Master，然后也通过Rc、Rs和PreMaster计算出EncryptKey。
>（7）客户端在发送信息后，此时已经具备对称加密的必要条件，即可以通过RandomC、RandomS和PreMaster计算出EncyptKey。然后将之前素有的通信参数的MD5值，通过EncryptKey加密发给服务端握手。
>（8）服务端也计算出之前所有参数收到的MD5值，并解出客户端的握手信息，两者进行比对，成功则OK。
>（9）双方SSL握手成功后，即可公用一个秘钥使用对称加密算法进行通信。
https://blog.csdn.net/u012545728/article/details/80548168
