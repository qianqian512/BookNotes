...
#### 20.TCP的KeeyAlive机制
> 1. tcp_keepalive_time 7200：代表7200秒后，开始进行探活检测。  
> 2. tcp_keepalive_intvl 75：每次间隔75秒，发送一个探活包。  
> 3. tcp_keepalive_probes 9：如果9个探活包都没有收到对端回应，则认为失去连接。  
> 扩展说明：虽然TCP提供了KeepAlive机制，但是默认是关闭状态，且整个探活过程需要2小时11分钟才会得出判定结果（虽然可以通过参数调整，但该参数是系统参数，一旦更改，OS创建的Socket连接都会受到影响）
...
>> 猜测：可能是为了预防回放攻击。如果以客户端生成随即秘钥方式（即直接生成Pre-Master通过publicKey加密交给Server），那么这种Server端没有参与创建的随机数，Client端可以无限发送同一请求，形成回放攻击。
> (2) 如果Server也需要Client的证书，那么会在上面流程2后，Client将自己的证书发给Server。
> (3) 如何防止证书被篡改？主要是通过数字签名，只有通过私钥才能创建签名，客户端用公钥来验证签名。
