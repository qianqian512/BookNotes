...
> &nbsp; &nbsp; &nbsp; &nbsp;第二个ACK:主动关闭方在收到fin后会回应ack，如果经过2msl后没有再次收到fin则closed（等待2msl的原因是因为主动关闭方要确保被动关闭方正确的断开连接，如果自己发的ack对端没有收到则对方还会再次发送fin给自己，而一回一发正好2msl，一回是指回复ack，一发是指如果被动关闭方如果没有收到ack包，则会再次重发一遍fin）；反之在2msl内再次收到fin请求，则重新回复ack，并重置time_wait倒计时。被动关闭方收到ack直接进入close，如果没有到ack，则会再次发送fin包告知对方。   
