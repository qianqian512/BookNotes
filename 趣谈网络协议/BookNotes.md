...
#### 5.MTU是1518的原因
> 这个值是考虑到传输效率和时间的一个折中方案，太小效率低，因为每次发送头尾就要18字节；而设置太大，因为以太网信号检测缘故，会导致阻塞等待时间过长
...
## 第7章：ICMP与Ping命令
#### 1.关于ICMP协议
> （1）ICMP全称是Internet Control Message Protocol，即互联网控制报文协议。属于OSI模型上的第三层协议，使用UDP协议传输数据包，一般不用于传输，而是用于分析网络情况，例如网络地址不可达，端口不可达，拥塞控制，ttl超时等。    
...
#### 3.一个udp包最大多大?
> 链路层mtu最大1518字节，链路层mac头尾18字节（头14字节，尾部校验和4字节），网络层ip头20字节，udp头8字节，最后udp数据部分就是1518-18-20-8=1472字节
...
#### 2.TCP4次挥手理解
> &nbsp; &nbsp; &nbsp; &nbsp;4次挥手其实就是两次FIN-ACK，只不过TCP是全双工的原因，每个方向都需要单独进行关闭，因此看上去就成了4次传输，实际是两次FIN+ACK。   

> &nbsp; &nbsp; &nbsp; &nbsp;第一个FIN:主动关闭方发fin意思是我没有数据要发送了，准备断开连接，进入fin_wait_1状态（此时的主动关闭方只是不发送数据了，但被动关闭方返回的数据客户端还是会处理）；  
> &nbsp; &nbsp; &nbsp; &nbsp;第一个ACK:被动关闭方收到fin后，知道主动关闭方没有数据要发送，于是也会根据自身情况选择是否也要关闭连接，但无论做什么，给主动关闭方回应一个ack是必要的，此时被动关闭方进入close_wait状态（服务端收到FIN包后会在接收缓冲区中插入一个eof文件结束符，应用程序通过read读取时来感知对端已经关闭连接）主动关闭方收到ack后会进入到fin_wait_2状态。  
> &nbsp; &nbsp; &nbsp; &nbsp;第二个FIN:被动关闭方认为没有数据要发送了，调用close发起第二个fin告知对方我没有数据要发送了，准备断开连接了，此时进入到last_ack状态。主动关闭方收到fin后会进入到time_wait状态。  
> &nbsp; &nbsp; &nbsp; &nbsp;第二个ACK:主动关闭方在收到fin后会回应ack，如果经过2msl后没有再次收到fin则closed；反之在2msl内再次收到fin请求，则重新回复ack，并重置time_wait倒计时。被动关闭方收到ack直接进入close，如果没有到ack，则会再次发送fin包告知对方。   

> &nbsp; &nbsp; &nbsp; &nbsp;极端场景1:由于fin_wait2没有类似于time_wait这种超时处理机制，那么如果只是自己关闭而对端不关闭的情况下，半连接会越来越多，直到系统内核crash，解决方案就是可以设置参数来调整fin_wait2超时时间，过期就关闭.   

> &nbsp; &nbsp; &nbsp; &nbsp;极端场景2:在第二次fin到达后，双方网络阻塞，不会收到任何包，此时双方怎么处理各自情况?能否正常关闭连接。其实对于主动关闭方是没有影响的，等2msl后没有收到新的fin就直接close了，而被动关闭方则会一直重试发送fin?重发若干次还没有ack直接关闭?

#### 2.1 挥手4次的原因，能否由Server端发起关闭？
> &nbsp; &nbsp; &nbsp; &nbsp;必要情况下，也可以由服务端发起关闭连接，从流程上讲无非就是Server端先发送FIN标识，对代码而言，也无非是ServerSocket先close而已。

#### 2.2 在主动关闭时为什么要等2MSL，而不是发送完ACK后立即关闭
> 1.保证tcp协议全双工连接可靠关闭；    
> 2.保证这个连接曾发送的数据短段全部从网络消失

#### 2.3 为什么要等2MSL，而不是1MSL或3MSL
> &nbsp; &nbsp; &nbsp; &nbsp;等待2msl的原因是因为主动关闭方要确保被动关闭方正确的断开连接，如果自己发的ack对端没有收到则对方还会再次发送fin给自己，而一回一发正好2msl，一回是指回复ack，一发是指如果被动关闭方如果没有收到ack包，则会再次重发一遍fin
...
#### 11.1 tcp的超时时间是怎么计算的
> &nbsp; &nbsp; &nbsp; &nbsp;tcp协议在传输过程中双方都会经历发送接收确认（体现在syn和ack不断的变化中），这一次往返经过的时间称之为rtt（往返时间），往返时间的计算公式是采用加权平均法计算（最新的rtt值依赖历史rtt值，随时间推移逐渐平滑）；而超时时间用rto表示，为了保证包正确到达，rto值既不能过大也不能太小，一般设置为比rtt稍微大一点点（过小会导致频繁重传增加网络压力，过长会导致收不到ack确认，性能下降）

#### 11.2 重传机制
> &nbsp; &nbsp; &nbsp; &nbsp;TCP的重传机制保证了在传输过程中数据的可靠性，TCP的重传一般发生在数据包(SYN)丢失或确认(ACK)应答丢失。     
> &nbsp; &nbsp; &nbsp; &nbsp;在发送数据时，会启动计数器监听每个数据包是否到达对端，如果计数器在耗尽时还未收到ACK，则会触发重传机制。弊端是如果等每个包的超时计数器溢出才触发重传，效率过低。假如在传输过程中只有中间一个包丢失，后面包接收都正常，我们期望发送方能第一时间感知到，因此才有了快速重传机制，以数据驱动代替时间驱动。在实际场景中定时器和快速重传是并行存在的。

#### 11.3 快速重传
> &nbsp; &nbsp; &nbsp; &nbsp;快速重传的目的是在传输多个数据包中，假如中间部分某一个包丢失，期望能够做到接收方希望发送方快速重发，而不是等待计时器超时才触发，从而提高传输效率。    
> &nbsp; &nbsp; &nbsp; &nbsp;做法是接收方为了让发送方第一时间感知到自己的丢包情况，约定发送方在连续收到3个或3个以上的冗余ack（算上正常ack实际为4个）时才会重传丢失的报文段。但快速重传只解决了超时问题，但仍面临另外一个问题，就是只单独重传丢失的包，还是重传丢包后面所有的包?针对这个问题又有了sack方法（选择性确认）。

#### 11.4 快速重传快速重传的触发为什么需要3次冗余应答
> &nbsp; &nbsp; &nbsp; &nbsp;首先3次也是一个估值，首先之所以需要在冗余应答触发，是因为tcp数据包在ip层传输时会有乱序的现象，以发送4个连续的数据包为例，在没丢包的情况下，有40%的可能出现3次冗余ACK，在乱序的情况下必定是2次冗余ACK ，在丢失的情况下，必定出现3次冗余ACK，基于这样的概率，选定3次冗余ACK作为阈值也算是合理的。在实际抓包中，大多数的快速重传都会在大于3次冗余ACK后发生。
>![](TCP retransfer_1.png)

#### 11.5 关于SACK和DSACK
> &nbsp; &nbsp; &nbsp; &nbsp;为什么要用SACK：SACK机制使TCP只重新发送丢失的包，不用发送后续所有的包，而且提供相应机制使接收方能告诉发送方哪些数据丢失，哪些数据重发了，哪些数据已经提前收到等。    
> &nbsp; &nbsp; &nbsp; &nbsp;SACK是如何工作的：SACK信息是通过TCP头的选项部分提供的，信息分两种，一种标识是否支持SACK, 在TCP握手时发送（kindtype=4）；另一种是具体的SACK信息（kindtype=5），因为tcp选项长度可变，但由于整个TCP选项长度不超过40字节，实际最多不超过4组边界值。在每次通信时，发送方会告诉接收一方，自己窗口ack不连续的的情况。Left Edge of Block不连续块的第一个数据的序列号，Right Edge of Block，不连续块的最后一个数据的序列号之后的序列号。   
> &nbsp; &nbsp; &nbsp; &nbsp;D-SACK有什么用：在接收时收到了重复的数据包，会用dsack告知发送方，要求按照接收方的ack情况发送未发过的包。dsack可以让发送方知道，是发送的包丢了，还是ack的包丢了。
...
> &nbsp; &nbsp; &nbsp; &nbsp;流量控制仅能保证点对点的流量控制，而拥塞控制是一个全局性的过程。换言之A和B主机网络通信情况良好，可以接受较快的发送/接收速率，但由于周围主机也消耗了比较高的带宽，甚至打满带宽时，导致A和B无法进行正常通信，这就是拥塞控制的理由。举个极端例子，网络内10台主机，出口1M带宽，如果不做控制，1台主机占满这个带宽后，其余9台主机则无法与外界通信；此时我们更希望是10台主机平分1M带宽，拥塞控制就是为了解决这样一个全局的问题。
...
> 使用场景
>> 对于一些需要小包场景的程序——比如像telnet或ssh这样的交互性比较强的程序，你需要关闭这个算法。你可以在Socket设置TCP_NODELAY选项来关闭这个算法（关闭Nagle算法没有全局参数，需要根据每个应用自己的特点来关闭）
...
#### 20.TCP重传机制
> TCP超时重传有4个地方配置：  
>> 第一次握手重试次数（tcp_syn_retries）默认是5次   
>> 第二次握手重试次数（tcp_synack_retries）默认是5次   
>> 第三次握手重试次数（tcp_ack_retries）默认是5次   
>> 传输时重试次数（tcp_retries）默认是15次   
>> 每次重传时间间隔为上次的2倍，1，2，4，8，16...

#### 21.tcp可以绕过3次握手吗
> 可以，但必须客户端服务端双方都要配置tcp_fastopen才可以。

#### 22.tcpdump&iptables
> 数据包进入linux系统入的顺序是先经过tcpdump，后经过iptables，出的顺序是先经过iptables再经过tcpdump。

#### 23.close_wait过多原因 
> &nbsp; &nbsp; &nbsp; &nbsp;被动关闭方的状态，收到对端的fin并回复了ack标记，自己进入close_wait状态，果服务端此时无事可做也应该发送fin标记给对方，让自己进入last_ack状态，因此按理说close_wait只是一个中间状态，出现大量的原因是因为客户端要求关闭连接，而服务端还忙于读或写操作，没有关闭连接，或者是压根忘记关闭连接导致

#### 24.time_wait过多的原因
> 主动关闭连接一方才会产生过多连接，而过多原因一般是因为短链接导致，且请求量多大，time_wait生产速度大于消耗速度。解决方案.   
>> 1.代码层面考虑使用长链接.   
>> 2.使用socket选项so_linger，调用关闭时不进行4次挥手，直接给对端发送rst标志位，不推荐使用   
>> 3.修改服务器配置（tcp_tw_reuse），让服务端在调用connect时可以复用time_wait超过1秒的连接   
>> 4.修改服务器配置（tcp_max_tw_bucket）一旦time_wait数量超过指定阈值，则立即全部回收，过于暴力，不推荐使用   

#### 25.tcp校验和机制
> &nbsp; &nbsp; &nbsp; &nbsp;将要发送的数据段当做16位（为什么是16位）的整数进行累加，最高位进位时面临越界时不做丢弃，将进位补到最低位加1，最后取反获得校验和（最后为什么要取反）。

#### 26.TCP理论最大连接数
> &nbsp; &nbsp; &nbsp; &nbsp;2的48次方=ip数2的32次方乘以端口数2的16次方，因为文件描述符和内存限制，tcp连接数远远未能达到上限

#### 27.TCP和UDP分片策略
> TCP的数据如果大于mss则在传输层分片，在到达对端后由传输层重组，中间丢失数据包再重传丢失的包即可；    
> UDP不具备分包传输功能，因为需要交给下层网络层拆包传输，一旦丢包需要重传整个传输层的报文，因为一般udp的报文应该小于mtu

#### 28.滑动窗口是如何工作的
> &nbsp; &nbsp; &nbsp; &nbsp;滑动窗口属于缓冲区的一部分，用来暂时存放收到的字节或准备发送的字节。发送和接受方各自维护一个窗口，并通过传输tcp头告知彼此自己窗口大小，来协商传输速率。当发送方窗口最左边节点得到ack后，即认为发送完成，窗口进行向右滑动，准备发送新的数据；接收方窗口最左边节点交给上层协议处理后，就向右滑动继续接收新的报文。

#### 29.滑动窗口解决了什么问题
> &nbsp; &nbsp; &nbsp; &nbsp;1.提高传输效率，一次可以发送多个数据包，无需阻塞等待每一个ack。     
> &nbsp; &nbsp; &nbsp; &nbsp;2.发送方的速率受接收方控制，避免发送方发送过快导致接收方缓存溢出。

#### 30.发送窗口上限值怎么算出
> swnd=min(rwnd,cwnd)。所以可以看出，发送窗口的速率实际上是由接收方的接受能力和网络拥塞情况所限制。

#### 30.1 接收窗口和发送窗口相等吗
> 不会，因为随着数据的传输，发送速率也在不断变化，因此两者之间永远都存在延迟，只能说是约等于关系。

#### 31.零窗口与探测器
> &nbsp; &nbsp; &nbsp; &nbsp;在接收方处理速度较慢的情况下，会告知发送方win=0 即停止发送，发送方收到后会暂时停止发送，等接收方处理完数据后通知新的窗口大小。但假如接收方新的通知窗口丢失时，即会出现两边都互相等待的死锁现象，为了避免这种情况，发送方在接到零窗口时会启动一个定时器，间隔去检测接收方新的窗口大小，如果检测若干次还是为零则发送rst包断开连接。

#### 32.什么是糊涂窗口综合征
> &nbsp; &nbsp; &nbsp; &nbsp;传输过程中，由于一方处理能力弱，需要降低窗口大小，导致窗口越来越小，最后仅有几个字节大小，极端情况荷载数据仅有一个字节，此时每次发送一个tcp数据包方内容数据还不如tcp头部大。    
> &nbsp; &nbsp; &nbsp; &nbsp;<b>解决糊涂窗口综合征</b>    
> &nbsp; &nbsp; &nbsp; &nbsp;针对接收方：当窗口大小小于mss的一半时，不再发送报文，直接发送一个零窗口通知，等窗口大小回复到mss一半以上再进行传输。    
> &nbsp; &nbsp; &nbsp; &nbsp;针对发送方：启用延迟发送，当数据或窗口大于mss时才发送；或者收到之前ack的回报也可以。如过不满足，则一直继续囤积数据。

#### 33.为什么需要拥塞控制
> &nbsp; &nbsp; &nbsp; &nbsp;如果再不加以控制的情况下，每个tcp连接都尽自己最大能力发送数据包，就会出现网络拥堵的情况，当网络环境变得糟糕后，各个tcp连接又会进行重试发包，然后网络会变得更加拥堵 进入恶性循环。于是tcp就被设计成一个无私的协议，一旦发现网络环境拥堵，就会自我牺牲，降低发送数据包的数量。

#### 34.tcp如何进行拥塞控制的
> &nbsp; &nbsp; &nbsp; &nbsp;tcp规定发送方维护了一个拥塞窗口cwnd，该窗口与网络拥塞程度实时变化。大的原则是网络中没有拥塞，cwnd就会增大，反之就会减少。那么引入一个新的问题，局部如何感知全局网络的拥塞?其方法是只要发送方在规定时间内没有收到ack，触发了超时重传就认为网络出现了拥塞。

#### 35.拥塞控制用了哪些算法
> &nbsp; &nbsp; &nbsp; &nbsp;慢启动：定义一个阈值，在小于这个阈值内每次发包指数递增，超过阈值后线性递增，一旦出现超时就重设阈值，令等于cwnd/2，然后重新进入慢启动。   
> &nbsp; &nbsp; &nbsp; &nbsp;快速重传：如果在接收ack时，发现只是个别丢包现象，不是网络拥塞导致，则领阈值等于cwnd/2，但cwnd等于阈值(不是直接赋予零，这点与慢启动不同)。区分网络拥塞与丢包的方法是，判断网络不好不会收到ack，而丢包则会收到丢失包后面序号重复的ack(一般还是认为3次)

#### 36.如何进行拥塞检测
> &nbsp; &nbsp; 1.根据丢包情况，丢包一般有两种原因，一是网络不好，二可能因为传输错误，因此也无法直接判断    
> &nbsp; &nbsp; 2.根据rtt变化:rtt明显增大时，则认为网络拥堵，但rtt测量也没有精确的方法   
> &nbsp; &nbsp; 3.显式的收到了路由器发来的拥塞通知(ip数据包enc标志位)，但目前没有得到普遍推广
...
#### 4.关于HTTPS思考
> (1) tls握手时为什么要搞3个随机数？一个不可以吗？  
>> 猜测：可能是为了预防回放攻击。如果以客户端生成随即秘钥方式（即直接生成Pre-Master通过publicKey加密交给Server），那么这种Server端没有参与创建的随机数，Client端可以无限发送同一请求，形成回放攻击。  客户端参与生成随机数应该是为了防止中间人吧？服务端参与生成，应该是为了防止回访攻击？好像都不对。。。。   
>> 大佬关于为什么不直接使用PreMaster给的答案：https://security.stackexchange.com/questions/89383/why-does-the-ssl-tls-handshake-have-a-client-and-server-random.  
> (2) 如果Server也需要Client的证书，那么会在上面流程2后，Client将自己的证书发给Server。   
> (3) 如何防止证书被篡改？主要是通过数字签名，只有通过私钥才能创建签名，客户端用公钥来验证签名。【中间人拦截后，不可以通过自己的私钥创建证书吗？】
